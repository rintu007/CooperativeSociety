...
<table class="table table-bordered table-striped">
    ...
    <tr>
        ...
        <th>
         ...
         <th>
            <a href="javascript:ajaxLoad('riskfund/list?field=AccountNo&sort={{Session::get("riskfund_sort")=="asc"?"desc":"asc"}}')">
                Account No
            </a>
            <i style="font-size: 12px"
               class="glyphicon  {{ Session::get('riskfund_field')=='AccountNo'?(Session::get('riskfund_sort')=='asc'?'glyphicon-sort-by-alphabet':'glyphicon-sort-by-alphabet-alt'):'' }}">
            </i>
        </th>
    ...
    @foreach($riskfunds as $key=>$riskfund)
        <tr>
            ...
            <td>{{$riskfund->AccountNo}}</td>
           <!--  
            <td>{{$riskfund->DPS}}</td>
            <td>{{$riskfund->FDR}}</td> -->
            <!-- <td align="right"><!-- $  --><!-- {{$riskfund->passbook}}</td> -->
           <!-- <td>{{$riskfund->share_amount}}</td>
            <td>{{$riskfund->saving_amount}}</td> -->
